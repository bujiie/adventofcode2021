    ...
    while len(subject) > 0:
        ...
        if typ == 4:
            ...
            if typ == 0: # sum
                ...
                result = min([r[-1] for r in results])
            elif typ == 3:
                result = max([r[-1] for r in results])
            elif typ == 5:
                result = 1 if results[0][-1] > results[1][-1] else 0
            elif typ == 6:
                result = 1 if results[0][-1] < results[1][-1] else 0
            elif typ == 7:
                result = 1 if results[0][-1] == results[1][-1] else 0
            mem.append((ver, typ, result))
